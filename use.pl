...
equip_weapon(A) :- equip(X), weapon_ammo(X, _), /*ammo(Z, _), *//*ammo(N), N>0,asserta(inventory(Z)),*/ retract(equip(X)),
...
/*equip_weapon(ularPython) :- equip(X), weapon_ammo(X, Z), ammo(Z, N), *//*ammo(N), N>0,asserta(inventory(Z)),*/ /*retract(equip(X)),
